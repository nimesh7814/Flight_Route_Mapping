...
```{r}
#| fig-cap: "Global airport locations from OpenFlights database"
#| fig-alt: "World map showing all airports as yellow points"
...
airports_map <- tm_shape(world) + 
  ...
  tm_dots(
    col = "#FDE725",  # Bright yellow from viridis
    size = 0.02,
    alpha = 0.6,
    border.lwd = 0
  ) + 
...
# Save the map
tmap_save(airports_map, "output/airports_map.png", width = 16, height = 10, dpi = 300)
...
src_count <- routes_joined %>% 
  count(Source_airport) %>% 
  rename(IATA = Source_airport, src_count = n)

dst_count <- routes_joined %>% 
  count(Destination_airport) %>% 
  rename(IATA = Destination_airport, dst_count = n)
...
```{r}
#| fig-cap: "Global airplane routes shown as straight (Euclidean) lines"
#| fig-alt: "World map with white flight routes connecting airports, with airport dots sized by traffic"
...
# Save the map
tmap_save(routes_lines_map, "output/routes_euclidean_map.png", width = 16, height = 10, dpi = 300)
...
```{r}
# Create geodetic lines using geosphere package
  ...
  mutate(
    geometry = {
      ...
      gc_route <- gcIntermediate(
        c(src_lon, src_lat), 
        c(dst_lon, dst_lat), 
        n = 100,  # 100 intermediate points
        addStartEnd = TRUE
      )
      
      # Convert to linestring
      st_linestring(gc_route)
    }
  ) %>%
  ungroup() %>%
  st_as_sf(crs = 4326) %>%
...
```{r}
#| fig-cap: "Global airplane routes shown as geodetic (curved) lines following Earth's curvature"
#| fig-alt: "World map with curved white flight routes representing actual flight paths"
...
# Save the map
tmap_save(routes_geodetic_map, "output/routes_geodetic_map.png", width = 16, height = 10, dpi = 300)
...
Create a hexagonal grid overlay for the world:

```{r}
# Create hexagonal grid (100km cell size)
hexgrid <- st_make_grid(
  world,
  cellsize = 1e5,  # 100,000 meters = 100 km
  what = "polygons",
...
#| fig-width: 16
#| fig-height: 10
#| fig-cap: "Global flight route density showing areas with highest air traffic"
#| fig-alt: "Heat map showing flight route density with bright colors indicating high traffic"

# Create the final map using tmap
map_geodetic <- tm_shape(world) +
  ...
  tm_polygons(
    ...
    labels = c("0-10", "10-50", "50-100", "100-250", "250-500", "500-1000", "1000-2500", "2500+"),
  ...
  tm_credits(
    text = "Using geodetically accurate curved routes that follow Earth's curvature\nData: OpenFlights | Routes follow great circle paths | Hexagon size: 100km",
    position = c("center", "bottom"),
    size = 0.6,
    col = "gray40"
  )
...
tmap_save(
  map_geodetic,
  "output/flight_density_map_final.png",
  width = 16,
  height = 10,
  dpi = 300
)
...
✅ How to create professional data visualizations with ggplot2
...
| `airports_map.png` | Map showing all airport locations |
...
| `routes_euclidean_map.png` | Map with straight routes and airports |
| `routes_geodetic_map.png` | Map with curved routes and airports |
| `flight_density_map_final.png` | Final heat map visualization |