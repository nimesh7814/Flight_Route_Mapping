...
install.packages(c(
  "readr", "dplyr", "stringr", "sf", "ggplot2", 
  "viridis", "scales", "rnaturalearth", "rnaturalearthdata",
  "geosphere", "tmap"
))
```
...
## 🛫 Step 2: Create Geodetic (Curved) Route Lines
...
cat("🔄 Creating geodetically accurate curved routes...\n")
cat("⏱️ This may take 2-3 minutes for", nrow(routes_joined), "routes\n\n")
...
for (i in 1:nrow(routes_joined)) {
  ...
  if (i %% 5000 == 0) {
    cat("  ✓ Processed", i, "of", nrow(routes_joined), "routes\n")
  }
}

cat("\n✓ Geodetic route creation complete!\n")
```
...
cat("✓ Created", nrow(routes_geodetic_sf), "geodetic route lines\n")
```
...
cat("⏱️ This may take 1-2 minutes\n\n")
...
Your map should show bright clusters in:

1. **Eastern USA** - New York, Atlanta, Chicago
2. **Western Europe** - London, Frankfurt, Paris
3. **Eastern Asia** - Beijing, Tokyo, Singapore
4. **Middle East** - Dubai, Doha
